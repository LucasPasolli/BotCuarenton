...
module.exports.run = async (bot, message, args) =>{//async no espera a que se realice la instruccion anterior
    
    setTimeout(() => message.delete(), 5000)
    message.channel.send('Ahi te lo mande al privado REY').then(msg => {
        msg.delete({ timeout: 5000});
    })
    const ingredientesTorta = fs.readFileSync('storage/torta/ingredientes.txt', 'utf8');
